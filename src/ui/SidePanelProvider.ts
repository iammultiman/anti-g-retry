...
export class SidePanelProvider implements vscode.WebviewViewProvider {
    ...
    private async handleOpenCDPSettings(): Promise<void> {
        ...
        const selection = await vscode.window.showQuickPick([
            {
                label: '$(rocket) Auto-Create ragy Script',
                description: 'Automatically create ~/bin/ragy now',
                detail: `Creates the script instantly - just restart IDE using 'ragy' command`,
                action: 'auto-create'
            },
            {
            ...
            title: '🚀 CDP Setup Guide - Enable ragy Command',
        ...
        switch (selection.action) {
            case 'auto-create': {
                // Auto-create the ragy script in ~/bin
...
# ragy - Launch ${ideName} with CDP enabled for Anti-g Retry
...
`;
                    ...
                    if (binInPath) {
                        ...
                        const nextStep = await vscode.window.showInformationMessage(
                            `✅ Script created at ${wrapperPath}\n\nNext: Quit ${ideName} (Cmd+Q) and run 'ragy' in terminal`,
                            'Copy: ragy'
                        );
                        if (nextStep === 'Copy: ragy') {
                            await vscode.env.clipboard.writeText('ragy');
                            vscode.window.showInformationMessage('Copied! Now quit IDE and run ragy in terminal.');
                        }
                    } else {
                        // Need to add ~/bin to PATH
                        const addPath = await vscode.window.showWarningMessage(
                            `✅ Script created at ${wrapperPath}\n\n⚠️ ~/bin is not in your PATH. Add it to use 'ragy' command.`,
                            'Copy PATH Setup',
                            'Skip (I\'ll handle it)'
                        );
                        ...
                        if (addPath === 'Copy PATH Setup') {
                            ...
                            vscode.window.showInformationMessage(
                                `PATH setup copied! After running it:\n1. Quit ${ideName} (Cmd+Q)\n2. Run 'ragy' in terminal`
                            );
                        } else {
                            vscode.window.showInformationMessage(
                                `Script ready! Add ~/bin to PATH, then run 'ragy' to start.`
                            );
                        }
                    }
                } catch (e: any) {
            ...
            case 'create-script': {
                ...
                if (os.platform() === 'darwin' && macAppPath) {
                    scriptContent = `#!/bin/bash
# ragy - Launch ${ideName} with CDP enabled for Anti-g Retry
# Install: mkdir -p ~/bin && pbpaste > ~/bin/ragy && chmod +x ~/bin/ragy
