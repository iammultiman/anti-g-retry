...
import { ServerInfo } from './server-detector';
...
export interface KeyModelQuota {
    claude: number;      // Claude remaining %
    geminiPro: number;   // Gemini Pro remaining %
...
/**
 * Extract key model quotas from snapshot
 */
export function extractKeyModels(snapshot: QuotaSnapshot): KeyModelQuota {
    const result: KeyModelQuota = {
        claude: -1,      // -1 means not available
        geminiPro: -1,
        geminiFlash: -1,
    };
